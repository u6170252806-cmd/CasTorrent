...
try:
	import socks  # PySocks for SOCKS proxies in requests test
	_HAS_PYSOCKS = True
except Exception:
	_HAS_PYSOCKS = False
...
def apply_proxy_settings(session, config):
	try:
		enabled = bool(config.get('proxy_enabled', False))
		sp = lt.settings_pack()
		if not enabled or not str(config.get('proxy_hostname', '')).strip():
			try:
				sp.set_int(lt.settings_pack.proxy_type, getattr(lt.settings_pack, 'none', 0))
		...
		proxy_type_str = str(config.get('proxy_type', 'socks5')).lower().strip()
		type_map = {
			'none': getattr(lt.settings_pack, 'none', 0),
			'socks4': getattr(lt.settings_pack, 'socks4', 1),
			'socks5': getattr(lt.settings_pack, 'socks5', 2),
			'socks5_pw': getattr(lt.settings_pack, 'socks5_pw', 3),
			'http': getattr(lt.settings_pack, 'http', 4),
			'http_pw': getattr(lt.settings_pack, 'http_pw', 5)
		}
		proxy_type = type_map.get(proxy_type_str, getattr(lt.settings_pack, 'socks5', 2))

		host = str(config.get('proxy_hostname', '') or '')
		port = int(config.get('proxy_port', 1080) or 1080)
		username = str(config.get('proxy_username', '') or '')
		password = str(config.get('proxy_password', '') or '')
		use_for_peers = bool(config.get('proxy_peer', True))
		use_for_trackers = bool(config.get('proxy_tracker', True))
		proxy_hostnames = bool(config.get('proxy_hostnames', True))
		force_proxy = bool(config.get('proxy_force', False))
		anonymous_mode = bool(config.get('proxy_anonymous_mode', True))
		...
		sp.set_str(lt.settings_pack.proxy_username, username)
		sp.set_str(lt.settings_pack.proxy_password, password)
		sp.set_bool(lt.settings_pack.proxy_peer_connections, use_for_peers)
		sp.set_bool(lt.settings_pack.proxy_tracker_connections, use_for_trackers)
		sp.set_bool(lt.settings_pack.proxy_hostnames, proxy_hostnames)
		sp.set_bool(lt.settings_pack.force_proxy, force_proxy)
		if hasattr(lt.settings_pack, 'anonymous_mode'):
			sp.set_bool(lt.settings_pack.anonymous_mode, anonymous_mode)

		session.apply_settings(sp)
		logger.info(f"Proxy applied: {proxy_type_str} {host}:{port} peers={use_for_peers} trackers={use_for_trackers}")
	except Exception as e:
		logger.warning(f"Failed to apply proxy settings: {e}")

def test_proxy_external_ip(proxy_conf: dict) -> tuple:
	try:
		host = str(proxy_conf.get('proxy_hostname', '') or '').strip()
		port = int(proxy_conf.get('proxy_port', 0) or 0)
		ptype = str(proxy_conf.get('proxy_type', 'socks5')).lower().strip()
		user = str(proxy_conf.get('proxy_username', '') or '')
		pw = str(proxy_conf.get('proxy_password', '') or '')

		if not host or not port:
			return (False, "Proxy host/port missing")

		proxies = {}
		if ptype in ('http', 'http_pw'):
			auth = f"{user}:{pw}@" if (user or pw) else ""
			base = f"http://{auth}{host}:{port}"
			proxies = {'http': base, 'https': base}
		elif ptype in ('socks5', 'socks5_pw', 'socks4'):
			if not _HAS_PYSOCKS:
				return (False, "PySocks not installed; cannot test SOCKS. pip install pysocks")
			scheme = 'socks5' if 'socks5' in ptype else 'socks4'
			auth = f"{user}:{pw}@" if (user or pw) else ""
			base = f"{scheme}://{auth}{host}:{port}"
			proxies = {'http': base, 'https': base}
		else:
			return (False, f"Unsupported proxy type for test: {ptype}")

		resp = requests.get("https://api.ipify.org?format=json", timeout=15, proxies=proxies)
		resp.raise_for_status()
		ip = resp.json().get('ip', 'unknown')
		return (True, f"External IP via proxy: {ip}")
	except Exception as e:
...
class TorrentClient:
	...
	def apply_initial_session_settings(self):
		...
		try:
			apply_proxy_settings(self.session, self.config)
		except Exception:
			pass
	...
	def apply_theme(self, dark: bool):
		try:
			...
			accent = "#3d7eff" if dark else "#1e5eff"
			sbg = "#1e1e1e" if dark else "#ffffff"
			hbg = "#232323" if dark else "#f0f0f0"
	...
	def toggle_theme(self):
		cur = bool(self.config.get('dark_mode', False))
		new = not cur
	...
	def create_menu(self):
		...
		view_menu.add_checkbutton(label="Dark Mode", variable=self.dark_mode_var, command=self.toggle_theme)
	...
	def create_toolbar(self):
		...
		ttk.Label(speed_frame, text="UL:").pack(side=tk.Left, padx=(5, 0)) if hasattr(ttk.Label, 'Left') else ttk.Label(speed_frame, text="UL:").pack(side=tk.LEFT, padx=(5, 0))
...
class SettingsDialog:
	def __init__(self, parent, app, config, apply_callback):
		...
		proxy_frame = ttk.Frame(notebook, padding="5 5 5 5")
	...
	def create_general_settings(self, parent):
		...
		self.dark_mode_var = tk.BooleanVar(value=bool(self.config.get('dark_mode', False)))
		ttk.Checkbutton(theme_row, text="Enable Dark Mode", variable=self.dark_mode_var).pack(anchor=tk.W)
	...
	def create_proxy_settings(self, parent):
		...
		ttk.Checkbutton(parent, text="Enable Proxy (mask IP for peers/trackers)", variable=self.proxy_enabled_var).pack(anchor=tk.W, pady=(0, 8))

		type_frame = ttk.Frame(parent)
		type_frame.pack(fill=tk.X, pady=(0, 8))
		ttk.Label(type_frame, text="Proxy Type:").pack(side=tk.LEFT)
		self.proxy_type_var = tk.StringVar(value=str(self.config.get('proxy_type', 'socks5')))
		type_combo = ttk.Combobox(type_frame, textvariable=self.proxy_type_var, state='readonly',
			values=['socks5', 'socks5_pw', 'socks4', 'http', 'http_pw', 'none'])
		type_combo.pack(side=tk.LEFT, padx=(6, 0))

		host_row = ttk.Frame(parent)
		host_row.pack(fill=tk.X, pady=(0, 6))
		ttk.Label(host_row, text="Hostname:").pack(side=tk.LEFT)
		self.proxy_host_var = tk.StringVar(value=str(self.config.get('proxy_hostname', '')))
		ttk.Entry(host_row, textvariable=self.proxy_host_var, width=28).pack(side=tk.LEFT, padx=(6, 10))
		ttk.Label(host_row, text="Port:").pack(side=tk.LEFT)
		self.proxy_port_var = tk.StringVar(value=str(self.config.get('proxy_port', 1080)))
		ttk.Entry(host_row, textvariable=self.proxy_port_var, width=8).pack(side=tk.LEFT, padx=(6, 0))

		auth_row = ttk.Frame(parent)
		auth_row.pack(fill=tk.X, pady=(0, 6))
		ttk.Label(auth_row, text="Username:").pack(side=tk.LEFT)
		self.proxy_user_var = tk.StringVar(value=str(self.config.get('proxy_username', '')))
		ttk.Entry(auth_row, textvariable=self.proxy_user_var, width=20).pack(side=tk.LEFT, padx=(6, 10))
		ttk.Label(auth_row, text="Password:").pack(side=tk.LEFT)
		self.proxy_pass_var = tk.StringVar(value=str(self.config.get('proxy_password', '')))
		ttk.Entry(auth_row, textvariable=self.proxy_pass_var, width=20, show="*").pack(side=tk.LEFT, padx=(6, 0))

		flags_frame = ttk.Frame(parent)
		flags_frame.pack(fill=tk.X, pady=(6, 6))
		self.proxy_peer_var = tk.BooleanVar(value=self.config.get('proxy_peer', True))
		self.proxy_tracker_var = tk.BooleanVar(value=self.config.get('proxy_tracker', True))
		self.proxy_hostnames_var = tk.BooleanVar(value=self.config.get('proxy_hostnames', True))
		self.proxy_force_var = tk.BooleanVar(value=self.config.get('proxy_force', False))
		self.proxy_anon_var = tk.BooleanVar(value=self.config.get('proxy_anonymous_mode', True))
		ttk.Checkbutton(flags_frame, text="Use for peer connections", variable=self.proxy_peer_var).pack(anchor=tk.W)
		ttk.Checkbutton(flags_frame, text="Use for tracker connections", variable=self.proxy_tracker_var).pack(anchor=tk.W)
		ttk.Checkbutton(flags_frame, text="Proxy hostnames (DNS via proxy)", variable=self.proxy_hostnames_var).pack(anchor=tk.W)
		ttk.Checkbutton(flags_frame, text="Force proxy (no direct connections)", variable=self.proxy_force_var).pack(anchor=tk.W)
		ttk.Checkbutton(flags_frame, text="Anonymous mode (reduce identifying info)", variable=self.proxy_anon_var).pack(anchor=tk.W)

		btns = ttk.Frame(parent)
		btns.pack(fill=tk.X, pady=(10, 0))
		ttk.Button(btns, text="Test Proxy IP", command=self.test_proxy_now).pack(side=tk.LEFT)
		ttk.Button(btns, text="Apply Now", command=self.apply_proxy_now).pack(side=tk.LEFT, padx=(8, 0))

	def test_proxy_now(self):
		cfg = {
			'proxy_enabled': self.proxy_enabled_var.get(),
			'proxy_type': self.proxy_type_var.get(),
			'proxy_hostname': self.proxy_host_var.get().strip(),
		...
		if not cfg['proxy_enabled']:
			messagebox.showinfo("Proxy Test", "Proxy is disabled. Enable it first.")
			return
		ok, msg = test_proxy_external_ip(cfg)
		if ok:
			messagebox.showinfo("Proxy Test", msg)
		else:
			messagebox.showerror("Proxy Test", msg)

	def apply_proxy_now(self):
		try:
			updated = self.app.config.copy()
			updated.update({
				'proxy_enabled': bool(self.proxy_enabled_var.get()),
				'proxy_type': str(self.proxy_type_var.get()).lower().strip(),
				'proxy_hostname': self.proxy_host_var.get().strip(),
				'proxy_port': max(1, min(65535, int(float(self.proxy_port_var.get()) if self.proxy_port_var.get() else 1080))),
				'proxy_username': self.proxy_user_var.get(),
				'proxy_password': self.proxy_pass_var.get(),
				'proxy_peer': bool(self.proxy_peer_var.get()),
				'proxy_tracker': bool(self.proxy_tracker_var.get()),
				'proxy_hostnames': bool(self.proxy_hostnames_var.get()),
				'proxy_force': bool(self.proxy_force_var.get()),
				'proxy_anonymous_mode': bool(self.proxy_anon_var.get())
			})
			apply_proxy_settings(self.app.session, updated)
			self.app.config.update(updated)
			self.app.save_config()
			messagebox.showinfo("Proxy", "Proxy settings applied to session.")
		except Exception as e:
			messagebox.showerror("Proxy", f"Failed to apply proxy: {e}")
